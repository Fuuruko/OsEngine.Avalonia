...
public class HorizontalVolume
{
    ...
    public void Save()
    {
        try
        {
            using StreamWriter writer = new(@"Engine\" + _name + @"HorizontalVolumeSet.txt", false);
    ...
    public void Load()
    {
        ...
        try
        {
            using StreamReader reader = new(@"Engine\" + _name + @"HorizontalVolumeSet.txt");
    ...
    public void Process(List<Candle> candles)
    {
        ...
        for (int i = index; i < candles.Count; i++)
        {
            ...
            if (cluster == null)
            {
                ...
                cluster.NewLineCreateEvent += VolumeClusterLines.Add;
            }
    ...
    /// </summary>
    public List<HorizontalVolumeLine> VolumeClusterLines = [];
...
public class HorizontalVolumeCluster
{
    ...
    List<HorizontalVolumeLine> _lines = [];
    ...
    /// </summary>
    public object _tradesArrayLocker = new();
            ...
            try
            {
                for (int i = 0; i < _tradesNew.Count; i++)
                {
                    ...
                    if (curLine == null)
                    {
                        ...
                        {
                            TimeFirstTrade = _tradesNew[i].Time,
                            Price = linePrice,
                            TimeCandle = Time,
...
public class HorizontalVolumeLine
{
    ...
    public string ToolTip
    {
        ...
        get
        {
            ...
            result.Append('.');
            result.Append(TimeCandle.Year);

            result.Append(' ');

            if (TimeCandle.Hour > 9)
            {
                result.Append(TimeCandle.Hour);
            }
            else
            {
            ...
            {
                result.Append(TimeCandle.Minute);
            }
            else
            {
            ...
            {
                result.Append(TimeCandle.Second);
            }
            else
            {
