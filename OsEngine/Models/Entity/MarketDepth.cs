...
public class MarketDepth
{
    ...
    public decimal AskSumVolume => Asks.Sum(a => a.Ask);
    public decimal BidSumVolume => Bids.Sum(b => b.Bid);

    /// <summary>
    /// Total sales volume
    /// </summary>
    [Obsolete("Use AskSumVolume instead or Asks.Sum(a => a.Ask)")]
    public decimal AskSummVolume => Asks.Sum(a => a.Ask);
    // {
    //     get
    //     {
    //         decimal vol = 0;
    ...
    /// Total buy volume
    /// </summary>
    [Obsolete("Use BidSumVolume instead or Bids.Sum(b => b.Bid)")]
    public decimal BidSummVolume => Bids.Sum(b => b.Bid);
    // {
    //     get
    //     {
    //         decimal vol = 0;
    ...
    public void SetMarketDepthFromString(string str)
    {
        ...
        for (int i = 0; i < bids.Length - 1; i++)
        {
            ...
            MarketDepthLevel newBid = new()
            {
                Ask = Convert.ToDecimal(val[0]),
                Price = Convert.ToDecimal(val[1])
            };
            Asks.Add(newBid);
        }
        ...
        for (int i = 0; i < asks.Length - 1; i++)
        {
            ...
            MarketDepthLevel newAsk = new()
            {
                Bid = Convert.ToDecimal(val[0]),
                Price = Convert.ToDecimal(val[1])
            };
            Bids.Add(newAsk);
        }
...
{
    // TODO: Union Ask and Bid to Volume?
    // Or Make it like mirror.
    // depth[0].Bid.Price, depth[0].Bid.Volume
    // depth[0].Ask.Price, depth[0].Ask.Volume
