...
namespace OsEngine.Models.Candles.Factory
{
    public static class CandleFactory
    {
        public static List<string> GetCandlesNames()
        {
            ...
            {
                resultOne[i] = resultOne[i].Split('\\')[^1];
        ...
        public static ACandlesSeriesRealization CreateCandleSeriesRealization(string nameClass)
        {
            ...
            try
            {
                if (_candlesTypes.TryGetValue(nameClass, out Type value))
                {
                    series = (ACandlesSeriesRealization)Activator.CreateInstance(value);
                }
        ...
        private static readonly List<ACandlesSeriesRealization> _compiledScriptInstancesCache = [];
        ...
        private static ACandlesSeriesRealization CompileAndInstantiateScript(string scriptPath, string nameClass)
        {
            ...
            if (!result.Success)
            {
                ...
                foreach (Diagnostic diagnostic in failures)
                {
                    ...
                    errorString += $"Location: {diagnostic.Location.GetLineSpan()}\n\n";
                    errorNum++;
                }
        ...
        private static List<NamesFilesFromFolder> _filesInDir = [];
        ...
        private static Dictionary<string, Type> GetCandlesTypesWithAttribute()
        {
            ...
            var types = Assembly.GetExecutingAssembly().GetTypes()
                .Where(t => baseType.IsAssignableFrom(t) && t.IsClass && !t.IsAbstract && t.IsPublic);
