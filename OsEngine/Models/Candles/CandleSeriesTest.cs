...
/// </summary>
// NOTE: Make CandleSeries basic class for Simple, HeikenAshi etc
// Then it can be unified TimeFrameBuilder CandleSeriesRealization and CandleSeries
public class CandleSeries2
{
    ...
    public CandleSeries2(TimeFrameBuilder timeFrameBuilder, Security security, StartProgram startProgram)
    {
        TimeFrameBuilder = timeFrameBuilder;
        timeFrameBuilder.CandleSeriesRealization.Security = security;
        // FIX: Will be broken when realisation changed
        timeFrameBuilder.CandleSeriesRealization.CandleUpdated += UpdateChangeCandle;
        timeFrameBuilder.CandleSeriesRealization.CandleFinished += UpdateFinishCandle;
        Security = security;
        _startProgram = startProgram;
    }

    public CandleSeries2(TimeFrame timeFrame, Security security, StartProgram startProgram)
    {
        TimeFrame = timeFrame;
        Security = security;
        _startProgram = startProgram;
    ...
    // TODO: Can be removed as not necessary
    ...
    /// </summary>
    public CandleMarketDataType CandleMarketDataType => TimeFrameBuilder.CandleMarketDataType;

    /// <summary>
    /// candle assembly type: simple, renko, delta, volume, etc
    /// </summary>
    public string CandleCreateMethodType => TimeFrameBuilder.CandleCreateMethodType;
    ...
    {
        get => TimeFrameBuilder.TimeFrame;
        set => TimeFrameBuilder.TimeFrame = value;
    }
    ...
    public List<Candle> CandlesOnlyReady
    {
        get
        {
            ...
            if (history[^1].State != CandleState.Finished)
            {
    ...
    public void PreLoad(List<Trade> trades)
    {
        ...
        foreach (Trade trade in trades)
        {
            ...
            UpDateCandle(trade.Time, trade.Price, trade.Volume, false, trade.Side);

            CandlesAll[^1].Trades.Add(trade);
        }
        TimeFrameBuilder.CandleSeriesRealization.CandleUpdated += UpdateChangeCandle;
        TimeFrameBuilder.CandleSeriesRealization.CandleFinished += UpdateFinishCandle;
        UpdateChangeCandle();
    ...
    public void LoadTradesInCandles(List<Trade> trades)
    {
        ...
        for (int i = 0; i < trades.Count; i++)
        {
            ...
            {
                // NOTE: Can be placed outside loop
                candleIndex = j;
                ...
                if (candle == null || nextCandle == null) { continue; }

                if (trade.Time >= candle.Time
                        && trade.Time < nextCandle.Time)
                {
                    candle.Trades.Add(trade);
                    break;
                }
    ...
    public void SetNewTime(DateTime time)
    {
        if (_isStoped
                ...
                || TimeFrameBuilder.CandleCreateMethodType != "Simple")
        {
            return;
        }
        ...
        if (lastCandle.Time.Add(TimeFrameSpan) < time.AddSeconds(-5))
        {
    ...
    public void SetNewTicks(List<Trade> trades)
    {
        ...
        try
        {
            Trade lastTrade = trades[^1];

            if (lastTrade == null || _lastTradeTime > lastTrade.Time) { return; }
        }
        catch
        {
            return;
        ...
        for (int i = 0; i < newTrades.Count; i++)
        {
            ...
            { // при классической сборке свечек. Когда мы точно знаем когда у свечи закрытие
                ...
                if (CandlesAll.Count > 0 &&
                    ...
                    CandlesAll[^1].Time.Add(TimeFrameSpan) > trade.Time)
                {
                    CandlesAll[^1].Trades.Add(trade);
                ...
                {
                    CandlesAll[^1].OpenInterest = trade.OpenInterest;
                }
            }
            else
    ...
    public void SetNewMarketDepth(MarketDepth marketDepth)
    {
        ...
        decimal price = (marketDepth.Asks[0].Price + marketDepth.Bids[0].Price) / 2;
    ...
    private List<Trade> GetActualTrades(List<Trade> trades)
    {
        ...
        { // если удаление трейдов из системы выключено
            ...
            if (newTradesCount <= 0) { return null; }
        ...
        else
        {
            ...
            else
            {
                ...
                for (int i = 0; i < trades.Count; i++)
                {
                    try
                    {
                        if (trades[i] == null || trades[i].Time < _lastTradeTime)
                        {
                            continue;
                        }
        ...
        if (newTrades.Count == 0) { return null; }

        _lastTradeTime = newTrades[^1].Time;

        _lastTradeIndexInTester = newTrades[^1].IdInTester;
    ...
    private List<string> _lastTradeIds = [];
    ...
    {
        return _lastTradeIds.Find(id_ => id_.Equals(id)) != null;
    }
    ...
    private void UpdateChangeCandle()
    {
        ...
        CandleUpdateEvent?.Invoke(this);
    }

    private void UpdateFinishCandle()
    {
        CandleFinishedEvent?.Invoke(this);
    }

    public event Action<CandleSeries2> CandleUpdateEvent;

    public event Action<CandleSeries2> CandleFinishedEvent;
    ...
    public void SetNewCandleInArray(Candle candle)
    {
        ...
        if (CandlesAll[^1].Time > candle.Time) { return; }

        if (CandlesAll[^1].Time == candle.Time)
        {
            CandlesAll[^1] = candle;
