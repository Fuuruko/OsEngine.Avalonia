...
namespace OsEngine.Models.Market.Servers.Finam.Entity;
...
public class FinamDataSeries
{
    ...
    public void Process()
    {
        ...
        else //if (IsTick == true)
        {
            ...
            for (int i = 0; trades != null && i < trades.Count; i++)
            {
                ...
                StreamReader reader = new(trades[i]);
    ...
    private List<Candle> GetCandles()
    {
        ...
        if (candles.Count != 0)
        {
            TimeActual = candles[^1].TimeStart;
        }
       ...
       // &from=01.03.2025&dt=1&mt=3&yt=2025&to=01.04.2025&p=7&f=RI.MOEXBC_250301_250401&e=.txt&cn=RI.MOEXBC&dtf=1&tmf=1&MSOR=1&mstime=on&mstimever=1&sep=1&sep2=1&datf=2&at=1
        ...
        if (response != "")
        {
            ...
            for (int i = 0; i < tradesInStr.Length; i++)
            {
                ...
                candles[^1].SetCandleFromString(tradesInStr[i]);
                candles[^1].TimeStart = candles[^1].TimeStart.Add(-_timeFrameSpan);

                if (candles.Count > 1)
                {
                    if (candles[^1].TimeStart <= candles[^2].TimeStart)
                    {
