...
namespace OsEngine.Models.Logging.Servers
{
    public class ServerTelegram
    {
        ...
        /// - Очередь сообщений
        /// </summary>
        private ConcurrentQueue<(string, Log)> _messagesQueue = new ConcurrentQueue<(string, Log)>();
        ...
        private void PullMessages()
        {
            ...
            while (true)
            {
                try
                {
                    ...
                    if(!_messagesQueue.TryDequeue(out (string, Log) msg)) 
                        continue;

                    // TODO:
                    // string message = msg.Item2.Message;
                    ...
                    // SendMessageAsync($"{msg.Item1} | {message}");
